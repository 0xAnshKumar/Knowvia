...
const SIDEBAR_COLLAPSED_KEY = "knowvia:sidebarCollapsed";
const RECENT_DOC_KEY = "knowvia:recentDocumentId";
...
export function WorkspaceShell({
  ...
  const documents = useQuery(
    api.documents.records.listDocuments,
    isAuthenticated ? {} : "skip",
  );
  const createDirectUploadTarget = useMutation(
    api.documents.uploadTargets.createDirectUploadTarget,
  );
  const completeDirectUpload = useAction(
    api.documents.uploads.completeDirectUpload,
  );
  const getDocumentPdfUrl = useAction(api.documents.uploads.getDocumentPdfUrl);
  const renameDocument = useMutation(api.documents.records.renameDocument);
  const retryDocumentProcessing = useMutation(
    api.documents.records.retryDocumentProcessing,
  );
  const deleteDocument = useAction(api.documents.records.deleteDocument);
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          <div className="flex items-center gap-3 border-b border-stone-800/60 px-4 py-2.5 lg:hidden">
            ...
            <span className="min-w-0 flex-1 truncate text-sm font-medium text-stone-300">
              {selectedDocument?.title ?? "Knowvia"}
            </span>
            {selectedDocument && (
              <MobileTabSwitcher
                activeTab={mobileTab}
              ...
              <div className="hidden min-h-0 flex-1 lg:flex">
                <div className="surface-base min-w-0 flex-1 border-r border-stone-800/60">
                  <PdfViewer
                    ...
                    onPageChange={(page) => {
                      setCurrentPage(page);
                      captureEvent("document_page_changed", {
                        document_id: selectedDocument._id,
                        page,
