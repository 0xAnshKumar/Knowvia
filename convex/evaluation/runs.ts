...
import { getChatConfig, streamStructuredAnswer } from "../chat/completion";
import {
...
import { EVALUATION_OWNER_TOKEN_IDENTIFIER } from "./constants";
...
export const runCase = internalAction({
  ...
  handler: async (ctx, args): Promise<EvaluationRunResult> => {
    const document: ReadyEvaluationDocument | null = await ctx.runQuery(
      internal.evaluation.data.getReadyCorpusDocumentForRun,
