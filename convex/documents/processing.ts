...
import { buildDocumentChunks, type DocumentChunk } from "./chunking";
import { createOpenAiEmbeddingClient, loadOpenAiChatConfig } from "../ai/openAi";
import { modelSupportsTemperature } from "../ai/modelCapabilities";
...
async function persistDocumentContent(
  ...
  await ctx.runMutation(internal.documents.records.clearDocumentPages, {
    documentId: document.documentId,
  });
  await ctx.runMutation(internal.documents.records.clearDocumentChunks, {
    documentId: document.documentId,
  });

  for (const batch of getBatches(pages, DOCUMENT_PAGE_BATCH_SIZE)) {
    await ctx.runMutation(internal.documents.records.insertDocumentPageBatch, {
      documentId: document.documentId,
  ...
  for (const batch of getBatches(chunks, EMBEDDING_REQUEST_BATCH_SIZE)) {
    await ctx.runMutation(internal.documents.records.insertDocumentChunkBatch, {
      documentId: document.documentId,
      ownerTokenIdentifier: document.ownerTokenIdentifier,
      chunks: batch,
    });
...
export const runDocumentOcr = internalAction({
  ...
  handler: async (ctx, args) => {
    const document = await ctx.runMutation(
      internal.documents.records.beginProcessingAttempt,
      {
    ...
    try {
      ...
      if (document.ocrResultStorageId) {
        ...
        await ctx.runMutation(internal.documents.records.recordOcrCheckpoint, {
          documentId: document.documentId,
          attemptNumber: args.attemptNumber,
      ...
      await ctx.runMutation(internal.documents.records.completeProcessingSuccess, {
        documentId: document.documentId,
        attemptNumber: args.attemptNumber,
        ocrMethod: result.method,
        ocrModel: result.model,
      ...
      if (canRetry) {
        await ctx.runMutation(internal.documents.records.markRetryPending, {
          documentId: document.documentId,
          attemptNumber: args.attemptNumber,
        });
        ...
        await ctx.scheduler.runAfter(
          retryDelay,
          internal.documents.processing.runDocumentOcr,
          {
            documentId: document.documentId,
      ...
      await ctx.runMutation(internal.documents.records.completeProcessingFailure, {
        documentId: document.documentId,
        attemptNumber: args.attemptNumber,
