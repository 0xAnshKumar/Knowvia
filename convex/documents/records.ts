...
export const deleteDocument = action({
  ...
  handler: async (ctx, args): Promise<null> => {
    ...
    await ctx.runMutation(internal.documents.records.deleteDocumentRecord, {
      documentId: args.documentId,
      ownerTokenIdentifier,
    });
    ...
    while (true) {
      const conversationIds = await ctx.runQuery(
        internal.chat.data.getDocumentConversationIds,
      ...
      for (const conversationId of conversationIds) {
        ...
        while (hasMore) {
          const result = await ctx.runMutation(
            internal.chat.data.deleteConversationMessagesBatch,
            { conversationId },
          );
          hasMore = result.hasMore;
        }
        await ctx.runMutation(internal.chat.data.deleteConversationRow, {
          conversationId,
        });
      }
    }
