  ...
  const response = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: `You prepare a standalone retrieval query for Knowvia, a document intelligence workspace.
...
export async function getChunkRetrievalTrace(
  ...
  if (args.pageNumber !== undefined) {
    const pageChunks: RetrievedChunk[] = await ctx.runQuery(
      internal.chat.data.getDocumentChunksForPage,
      {
  ...
  const lexicalSearchPromise: Promise<Id<"documentChunks">[]> = ctx.runQuery(
    internal.chat.data.searchDocumentChunks,
    {
      ownerDocumentKey,
  ...
  const chunks: RetrievedChunk[] = await ctx.runQuery(
    internal.chat.data.getDocumentChunks,
    {
  ...
  const neighborChunks: RetrievedChunk[] = await ctx.runQuery(
    internal.chat.data.getDocumentChunksByIndexes,
    {
      documentId: args.documentId,
      ownerTokenIdentifier: args.ownerTokenIdentifier,
