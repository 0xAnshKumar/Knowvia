...
import { getChatConfig, streamStructuredAnswer } from "./completion";
import {
...
export const streamChat = httpAction(async (ctx, req) => {
  ...
  const document = await ctx.runQuery(internal.documents.records.getOwnedDocument, {
  ...
  if (isRegenerate) {
    const conversation = await ctx.runQuery(
      internal.chat.data.getOwnedConversation,
      {
    ...
    const claim = await ctx.runMutation(internal.chat.data.startRegeneration, {
      conversationId,
    ...
    if (rawConversationId) {
      ...
      conversationId = await ctx.runMutation(
        internal.chat.data.createConversation,
        {
          ownerTokenIdentifier,
          documentId: documentId as Id<"documents">,
          title: queryContent,
    ...
    await ctx.runMutation(internal.chat.data.addMessage, {
      conversationId,
      role: "user",
    ...
    assistantMessageId = await ctx.runMutation(
      internal.chat.data.createStreamingAssistantMessage,
      { conversationId },
    );
  }

  const history: ConversationTurn[] = await ctx.runQuery(
    internal.chat.data.getConversationHistory,
    {
      conversationId,
      ownerTokenIdentifier,
  ...
  const hasChunkData = await ctx.runQuery(internal.chat.data.hasDocumentChunks, {
    documentId: documentId as Id<"documents">,
    ownerTokenIdentifier,
  });

  if (!hasChunkData) {
    await ctx.runMutation(internal.chat.data.finalizeAssistantMessage, {
  ...
  const isStillStreaming = async () => {
    const status = await ctx.runQuery(internal.chat.data.getMessageStatus, {
      messageId: assistantMessageId,
    });
      ...
      try {
        ...
        if (routing.retrievalMode === "summaries" && pageNumber === undefined) {
          const summaryContext = await ctx.runQuery(
            internal.chat.data.getDocumentSummaryContext,
            {
              documentId: documentId as Id<"documents">,
              ownerTokenIdentifier,
            },
